...
在正文中使用 `[^名称]` 标记脚注，并在段落外使用相同名称写出说明：

```markdown
这句话需要补充说明。[^example]

[^example]: 这里是脚注内容，可以包含链接和其他 Markdown 格式。
```
...
[^example]: 这里是脚注内容，可以包含[链接](https://example.com)和其他 Markdown 格式。
...
粘贴抖音分享链接即可生成点击后加载的播放器：

```markdown
@[douyin](https://v.douyin.com/分享码/)
```

仅支持公开的视频。播放器会根据视频原始尺寸自动选择横屏或竖屏布局；链接失效或无法解析时，会显示普通备用链接。
